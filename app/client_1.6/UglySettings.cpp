...
LastFmUserSettings::addRecentStation( const Station& station )
{
    ...
    s.setValue( "StationNames/" + station.url(), station.title() );
    s.sync();
...
LastFmUserSettings::recentStations()
{
    ...
    foreach (QString key, keys) {
        Station station( s.value( "RecentStations/" + key ).toString() );
        station.setTitle( s.value( "StationNames/" + station.url() ).toString() );
...
{
    return MyQSettings( this ).value( "ScrobblePoint", MooseConstants::kScrobblePoint ).toInt();
}
...
LastFmSettings::getFreeColour()
{
    ...
    foreach (QString username, s.childGroups())
    {
        ...
        if (unused.isEmpty()) {
            qWarning() << "We ran out of colours, returning random";
