...
class LastFmUserSettings : public UserSettings<QSettings>
{
    Q_OBJECT
...
public:
    QString currentUsername() const { return Ws::Username; }
...
//PHASED OUT    int  soundCard()     const { return QSettings().value( "soundcard", 0 ).toInt(); }
//PHASED OUT    void setSoundCard( int v ) { QSettings().setValue( "soundcard", v ); }

//PHASED OUT    int  soundSystem()     const { return QSettings().value( "soundsystem", 0 ).toInt(); }
//PHASED OUT    void setSoundSystem( int v ) { QSettings().setValue( "soundsystem", v ); }
